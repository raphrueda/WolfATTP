...
def make_chart(workbook, worksheet, company, DM, chart_type, x_col,\
    ...
    if x2_col and y2_col and table2_mark and table2_height and y2_axis_name:
        chart.add_series({
            'name': strategy,
            'values': '=' + company\
                    + '!'\
...
def create_summary_page(DM, workbook):
    ...
    for i in range(len(DM.strategies)):
        summary_worksheet.write('A' + str(curr_row_lvl) , DM.strategies[i] + ' TOTAL REVENUE:', right_align)
        curr_row_lvl += inc
    ...
    for key in DM.combined_revenue.keys():
        ...
        summary_worksheet.write_column(xl_rowcol_to_cell(temp_data_row + 1, temp_data_col), map(date_convertor, DM.combined_revenue[key]['dates']), date)
        summary_worksheet.write_column(xl_rowcol_to_cell(temp_data_row + 1, temp_data_col + 1), DM.combined_revenue[key]['pl_over_time'])
...
def strategy_performance_report_builder(DM):
    workbook = xlsxwriter.Workbook('Strategy_Performance_Report.xlsx')
