...
def csvTestPrinter(dataArray):

    with open('testing.csv','w') as f1:
        writer = csv.writer(f1, delimiter=',',lineterminator='\n',)
        #header = 'CompanyName,Date/Time,Price,Volume,Value,Bid/Ask'
        #writer.writerow(header)
        for company in range(len(dataArray)):
            for i in range(len(dataArray[company])):
                tempRow = []
                for m in range(len(dataArray[company][i])):
                    tempRow.append(dataArray[company][i][m])
...
def decomposeCSV():
    ...
    #     temp = 0
    #     for m in range(len(masterArray[i])):
    #         temp = temp +1
    #     print masterArray[i][0][1] + " " + str(temp)
    return  paramN, paramTH, numOfCompanys, masterArray

def generateReturns(dataArray, numOfCompanys, paramN):
...
def SMA(n, dataArray, numOfCompanys):
    ...
    for company in range(numOfCompanys):
        ...
        if length > n:
            for i in range(n):
                # print " i = : " + str(i) + " lenght of comapany: " + str(len(dataArray[company]))
                # smAvg.append(0)
            ...
            ##append 0 to all companys entries
            for m in range(length):
                dataArray[company][m].append(0)
...
def main():
    ...
    if (globalErrorList == []):
        dataArray = generateReturns(dataArray, numOfCompanys, paramN)
    if (globalErrorList == []):
